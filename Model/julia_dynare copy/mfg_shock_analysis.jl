...
ctx = (
    ...
    title_long = "-10% Manufacturing Productivity Shock", title_short = "Manufacturing TFP Shock",
    ...
    exposure_label = "Input cost share from Manufacturing (%)",
    exposure_ylabel = "Cost share (%)",
    exposure_title = "Input Cost Share Sourced from Manufacturing by Sector",
    share_vec = exposure_share, modalphaV = modalphaV, Yi_ss = Yi_ss,
    decomp_share_head = "Manuf.", decomp_share_sub = "Input (\%)",
    decomp_caption = "Sectoral Decomposition of a 10\% Negative TFP Shock in Manufacturing",
    ...
    agg_caption = "Aggregate Responses to a 10\% Negative TFP Shock in Manufacturing",
    agg_notes = "GDP, consumption, employment, and RER in \% deviations from SS; inflation and policy rate in annualized pp; trade balance in pp of GDP. The shock is a one-time 10\% reduction in sector-Manufacturing total factor productivity with persistence \$\rho = $(round(rho_tfp1_val,digits=3))\$. Impact = quarter 1 response. Trough/Peak = extreme response over 40 quarters.",
    get_irf = get_irf_var,
)
