...
@printf "\n%s\n" repeat("=", 70)
@printf "  NK-SOE 12-sector model — Mining TFP Shock Analysis (-10%%)\n"
@printf "%s\n\n" repeat("=", 70)
...
# ---- TFP SHOCK ONLY (sector 2 = Mining) ----
shock_sector = 2
sigma_i_val     = 0.0
rho_om1_val     = 0.1
...
@printf "--- Aggregate IRFs (-10%% Mining TFP shock, impact) ---
"
...
ctx = (
    ...
    title_long = "-10% Mining Productivity Shock", title_short = "Mining TFP Shock",
    ...
    exposure_label = "Input cost share from Mining (%)",
    exposure_ylabel = "Cost share (%)",
    exposure_title = "Input Cost Share Sourced from Mining by Sector",
    share_vec = exposure_share, modalphaV = modalphaV, Yi_ss = Yi_ss,
    decomp_share_head = "Mining", decomp_share_sub = "Input (\%)",
    decomp_caption = "Sectoral Decomposition of a 10\% Negative TFP Shock in Mining",
    ...
    agg_caption = "Aggregate Responses to a 10\% Negative TFP Shock in Mining",
    agg_notes = "GDP, consumption, employment, and RER in \% deviations from SS; inflation and policy rate in annualized pp; trade balance in pp of GDP. The shock is a one-time 10\% reduction in sector-Mining total factor productivity with persistence \$\rho = $(round(rho_tfp1_val,digits=3))\$. Impact = quarter 1 response. Trough/Peak = extreme response over 40 quarters.",
    get_irf = get_irf_var,
)
