...
OVERLEAF_FIG_DIR = joinpath(OVERLEAF_ROOT, "Figures", "agr_shock")
...
ctx = (
    ...
    title_long = "-10% Agriculture Productivity Shock", title_short = "Agriculture TFP Shock",
    ...
    overleaf_ok = overleaf_ok, fignames = standard_fignames("agr"),
    ...
    exposure_label = "Input cost share from Agriculture (%)",
    exposure_ylabel = "Cost share (%)",
    exposure_title = "Input Cost Share Sourced from Agriculture by Sector",
    share_vec = exposure_share, modalphaV = modalphaV, Yi_ss = Yi_ss,
    decomp_share_head = "Agric.", decomp_share_sub = "Input (\%)",
    decomp_caption = "Sectoral Decomposition of a 10\% Negative TFP Shock in Agriculture",
    decomp_notes = "Input Share is the fraction of sector \$i\$'s intermediate inputs sourced from Agriculture (Chile 2021 IO tables). Direct MC is the own-sector productivity cost push \$-\hat A_i\$, nonzero only for the shocked sector. Network is the additional cost propagated through IO linkages under the Leontief PE approximation. Total = Direct + Network. Amplification ratio = Total/Direct (shown only for the shocked sector). Output IRF is the impact-period sectoral output response. G\,=\,Goods, S\,=\,Services.",
    agg_caption = "Aggregate Responses to a 10\% Negative TFP Shock in Agriculture",
    agg_notes = "GDP, consumption, employment, and RER in \% deviations from SS; inflation and policy rate in annualized pp; trade balance in pp of GDP. The shock is a one-time 10\% reduction in sector-Agriculture total factor productivity with persistence \$\rho = $(round(rho_tfp1_val,digits=3))\$. Impact = quarter 1 response. Trough/Peak = extreme response over 40 quarters.",
    get_irf = get_irf_var,
)
...
@printf "\n%s\n" repeat("=", 70)
@printf "  Agriculture TFP shock analysis complete.\n"
