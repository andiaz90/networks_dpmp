...
function smm_run(context::Dynare.Context; endo_names_override=nothing)
    ...
    # ============ θ-REDUCTION: the only SMM — estimate the 8 transmission params,
    #             pin the 28 others (elasticities + 24 measured sectoral shocks + external).
    REDUCED = true
    if REDUCED
        θmeas = copy(θ0)
        θmeas[2] = parse(Float64, get(ENV, "SMM_EPSY", "0.80"))   # epsY (Atalay eps_Q)
        θmeas[3] = parse(Float64, get(ENV, "SMM_EPSM", "0.20"))   # epsM (Atalay eps_m)
        _scf = joinpath(DATA_DIR, "sectoral_shock_calibration.csv")
        if isfile(_scf)
            _sc = CSV.read(_scf, DataFrame)
            θmeas[7:18]  = Float64.(_sc.isigma_tfp_init)   # measured (may exceed old bounds — pinned, not searched)
            θmeas[19:30] = Float64.(_sc.sigma_om_init)
        end
        ...
        if isfile(_ecf)
            ...
            haskey(_em, "rho_pvstar")   && (θmeas[31] = _em["rho_pvstar"])
            haskey(_em, "sigma_pvstar") && (θmeas[32] = _em["sigma_pvstar"])
        end
        SMM_PIN[] = copy(θmeas)              # objective uses these exact values for the 28 pinned entries
        θ0 = clamp.(θmeas, LB, UB)           # CMA-ES seed: pinned dims clamped so scaled space stays [0,1]
        # 8 FREE transmission params seeded at sane INTERIOR values (never a bound):
        θ0[1]  = clamp(1.0,      LB[1],  UB[1])    # ilabcosts
        θ0[4]  = clamp(log(1e6), LB[4],  UB[4])    # log(kappaV)
        θ0[5]  = clamp(0.5,      LB[5],  UB[5])    # rho_om
        θ0[6]  = clamp(0.5,      LB[6],  UB[6])    # rho_A
        θ0[33] = clamp(0.5,      LB[33], UB[33])   # rho_xi
        θ0[34] = clamp(0.015,    LB[34], UB[34])   # sigma_xi   (stale ckpt had it at UB)
        θ0[35] = clamp(2.0,      LB[35], UB[35])   # etastar    (stale ckpt had it at UB)
        θ0[36] = clamp(100.0,    LB[36], UB[36])   # kappaw
        @printf "  [reduced] θ-reduction ON: 8 transmission params estimated, 28 pinned.\n"
        @printf "  [reduced] pins: epsY=%.2f epsM=%.2f | 24 measured sectoral shocks | rho_pvstar=%.3f sigma_pvstar=%.3f\n" θmeas[2] θmeas[3] θmeas[31] θmeas[32]
        @printf "  [reduced] free seeds: sigma_xi=%.3f etastar=%.2f ilabcosts=%.2f kappaw=%.0f\n" θ0[34] θ0[35] θ0[1] θ0[36]
    else
        REDUCED = false
    end
    ...
    if REDUCED
        ...
        for i in 1:N_THETA
            if !(i in _free)
                LB_sc[i] = max(0.0, θ0_sc[i] - 1e-9)   # freeze pinned dim (tiny band; objective overrides exactly)
