...
ctx = (
    ...
    title_long = "Shock de Productividad en Manufactura (-1%)", title_short = "Shock PTF Manufactura",
    ...
    decomp_caption = "Descomposición Sectorial de un Shock Negativo de $(round(abs(shock_pct)*100, digits=1))\\% a la PTF en Manufactura",
    ...
    agg_caption = "Respuestas Agregadas a un Shock Negativo de $(round(abs(shock_pct)*100, digits=1))\\% a la PTF en Manufactura",
    agg_notes = "PIB, consumo, empleo y TCR en \\% de desviación del estado estacionario; inflación y tasa de política en pp anualizados; balanza comercial en pp del PIB. El shock es una reducción única de $(round(abs(shock_pct)*100, digits=1))\\% en la productividad total de factores del sector Manufactura con persistencia \$\\rho = $(round(rho_tfp1_val[shock_sector],digits=3))\$. Impacto = respuesta en el trimestre 1. Mín/Máx = respuesta extrema en 40 trimestres.",
    get_irf = get_irf_var,
)
